...
void Drzewo::rotacjaWPrawo(DrzewoElement *element) {
	...
	if (y != &straznik) {
		...
		if (element->lewo != &straznik)
			element->lewo->rodzic = element;
...
DrzewoElement *Drzewo::wyszukajNastepnyElement(DrzewoElement *p) {
	...
	if (p != &straznik) {
		if (p->prawo != &straznik)
			return wyszukajNajmniejszyElementDrzewa(p->prawo);
		else {
			nastepnyElement = p->rodzic;
			while ((nastepnyElement != &straznik)
					&& (p == nastepnyElement->prawo)) {
				p = nastepnyElement;
...
void Drzewo::usun(int wartosc) {
	...
	if ((elementDoUsuniecia->lewo == &straznik)
			|| (elementDoUsuniecia->prawo == &straznik)) {
